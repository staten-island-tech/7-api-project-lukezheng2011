...
window = tk.Tk()
window.title("Harry Potter Book API Project")
window.geometry("600x400")
window.resizable(False, False)

prompt = tk.Label(window, text="Enter the Harry Potter book title (completely):", font=("Ariel", 14))
prompt.pack(pady=10)

entry = tk.Entry(window, font=("Ariel", 14), width=40)
entry.pack(pady=5)

search_button = tk.Button(window, text="Find Description", font=("Arial", 14), command=search_book)
search_button.pack(pady=10)

result_label = tk.Label(window, text="", font=("Ariel", 12), wraplength=550, justify="left", fg="blue")
